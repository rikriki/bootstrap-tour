...
(function(window, factory) {
	...
	Tour = (function() {
		...
		Tour.prototype._checkBackground = function() {
			var step =this.getStep(this.getCurrentStep())
			$element = $(step.element);
			$backdropElement = $(step.backdropElement);
			var previousDimensions = $backdropElement.data('dimensions');
			var elementData = {
			...
			if (previousDimensions) {
				...
				for (var i in elementData) {
					if (typeof elementData[i] == 'object') {
						for (var j in elementData[i]) {
							changed = Math.abs(elementData[i][j] - previousDimensions[i][j]) > 2
							if (changed)
								break;
						}
					} else {
						changed = Math.abs(elementData[i] - previousDimensions[i]) > 2
					}
					if (changed)
						break;
				}
			...
			$backdropElement.data('dimensions', {
